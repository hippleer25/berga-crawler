...
from berga_crawler.crawler.middleware.content_type import ContentTypeMiddleware
from berga_crawler.crawler.middleware.cookie import CookieMiddleware
from berga_crawler.crawler.middleware.monitoring import MonitoringMiddleware
...
from berga_crawler.crawler.trace import add_trace_config
...
class Crawler(ABC):
        ...
        try:
            while True:
                ...
                if item_wait_time := queue_item.get_queue_wait_time():
                    await self.stats_collector.record_queue_metrics(item_wait_time, qsize)
                ...
                try:
                    ...
                    elif isinstance(queue_item, Request):
                        await self._handle_request(queue_item)
    ...
    async def crawl(self, urls: Union[URL, str, List[Union[URL, str]]] = []) -> None:
        ...
        # Track root URLs for error reporting
        self._root_urls = {str(u) for u in initial_urls}
        self._root_url_error = None
