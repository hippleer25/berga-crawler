...
from berga_crawler.crawler.exceptions import RobotsBlockedException
...
class RobotsMiddleware(BaseDownloaderMiddleware):
    ...
    async def process_request(self, request: Request) -> None:
        ...
        try:
            ...
            # Don't block robots.txt or sitemaps
            url_str = str(request.url)
            if url_str.endswith("robots.txt") or url_str.endswith("sitemap.xml"):
                return
            ...
            if rp and not rp.can_fetch(self.user_agent, url_str):
                logger.debug(f"Blocked by robots.txt: {request.url}")
                raise RobotsBlockedException(url_str)

        except RobotsBlockedException:
            raise
