...
from berga_crawler.crawler import Crawler, Item, Request, Response
from berga_crawler.crawler.lib import parse_href_to_url
...
from berga_crawler.feed_spider.regexes import rss_regex
...
class FeedsearchSpider(Crawler):
    ...
    async def parse_response(
        ...
        # Avoid parsing sitemaps as normal pages
        if response.url.path.endswith("sitemap.xml") or response.url.path.endswith("sitemap_index.xml"):
            return

        # Parse SiteMeta only for original domain
        if response.is_original_domain():
    ...
    async def parse_response_content(
        self, response_text: str, request: Request = None, response: Response = None
    ) -> Any:
        """
